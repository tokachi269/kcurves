...
namespace Assets
{
    public class Path : MonoBehaviour
    {
        ...
        public void MoveCamera()
        {
            ...
            if (inputL >= KnotBetweenRange)
            {
                if (bezierIndex <= Knots.Count - 1)
                {
                    if (bezierIndex % 2 == 0)
                    {
                        knotIndex++;
            ...
            Debug.Log((bezierIndex % 2 == 0 ? inputL - Beziers.Lengths[2 * knotIndex - 1, Beziers.ArcLengthWithTStep - 1] : inputL) / KnotBetweenRange);
            float easing = Easing.GetEasing(mode, (bezierIndex % 2 == 0 ? inputL - Beziers.Lengths[2 * knotIndex - 1, Beziers.ArcLengthWithTStep - 1] : inputL) / KnotBetweenRange);
            Debug.Log(easing);
            t = Beziers.GetT(bezierIndex, ref easing * Beziers.Lengths[bezierIndex, Beziers.ArcLengthWithTStep - 1]);
