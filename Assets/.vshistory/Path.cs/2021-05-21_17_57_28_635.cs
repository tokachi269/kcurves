    ...
    public class Path : MonoBehaviour
    {
        ...
        public Quaternion CalcRotation(int segIndex, float inputL)
        {
            ...
            if (LookAts.Count != 0)
            {
                ...
                {
                    rotation = Vector3.Lerp((Knots[knotIndex].isLookAt ? (Quaternion.LookRotation(LookAts[0].position) * new Quaternion(-1, 0, 0, 1)).eulerAngles : rotation),
                                           (Knots[knotNextIndex].isLookAt ? Quaternion.LookRotation(LookAts[0].position) * new Quaternion(-1, 0, 0, 1)).eulerAngles : rotation), t);
                }
            }
            rotation.z = 0f;
