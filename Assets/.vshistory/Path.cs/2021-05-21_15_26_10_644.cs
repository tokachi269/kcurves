...
namespace Assets
{
    public class Path : MonoBehaviour
    {
        ...
        public Quaternion CalcRotation(int segIndex, float inputL)
        {
            ...
            {
                knotIndex = ((segIndex - 2) / 4 + 1) * 2 - 1;
            }

            int nextIndex = (knotIndex < Knots.Count ? segIndex + 1 : knotIndex);


            Quaternion rotation;
            if (!Quaternion.Equals(Knots[knotIndex].rotation, Knots[nextIndex].rotation))
            {
                rotation = Quaternion.Lerp(Knots[knotIndex].rotation, Knots[nextIndex].rotation, t);
            }
            else
            {
                rotation = Knots[knotIndex].rotation;
            }
            if (LookAts.Count != 0)
            {
                if (Knots[knotIndex].isLookAt || Knots[nextIndex].isLookAt)
                {
                    rotation = Quaternion.Lerp((Knots[knotIndex].isLookAt ? Quaternion.LookRotation(LookAts[0].position) : rotation),
