...
namespace Assets
{
    public class Path : MonoBehaviour
    {
        ...
        public void MoveCamera()
        {
            ...
            if (_bezierIndex % 2 == 1)
            {
                if (ProgressLength < Beziers.Length(_bezierIndex)) _bezierIndex++;
            }
            else if (_bezierIndex!=0 && ProgressLength < Beziers.Length(_bezierIndex - 1) + Beziers.Length(_bezierIndex)) 
            {
                _bezierIndex++;
                ProgressLength -= Beziers.Length(_bezierIndex - 1) + Beziers.Length(_bezierIndex);
            }


            float easing = Easing.GetEasing(mode, (_bezierIndex != 0 && _bezierIndex % 2 == 0 ? ProgressLength - Beziers.Length(2 * knotIndex - 1) : ProgressLength) / KnotBetweenRange);
            Debug.Log(easing);
