...
namespace Assets
{
    public class Path : MonoBehaviour
    {
        ...
        {                                                                           //5       
            dividedSegmentCount = (extendBezierControls.SegmentCount - 2 ) * 2 ; //6
            Vector3[] dividedPoints = new Vector3[dividedSegmentCount * 2 + 1]; //13
            ...
            dividedPoints[0] = extendBezierControls.Points[1];
            for (int i = 1; i < segCnt; i++)
            {
                Vector3[] result = BezierUtil.Divide(extendBezierControls[i+1, 0], extendBezierControls[i+1, 1], extendBezierControls[i+1, 2], (float)extendBezierControls.Ts[i+1]);

                for (int j = 0; j <= 3; j++)
                {
                    dividedPoints[4 * (i - 1) + j + 1] = result[j];
                }
                dividedPoints[dividedSegmentCount-1] = extendBezierControls.Points[extendBezierControls.Points.Length - 2];
        ...
        public Vector3[] Output(int step, bool isLoop)
        {
            ...
            if (extendBezierControls.SegmentCount > 3 || (extendBezierControls.SegmentCount != (Knots.Count < 3 ? 1 : Knots.Count)))
            {
                int segment;
                Vector3[] divVec3= DividePoints(isLoop, out segment);
                extendBezierControls = new ExtendBezierControls(segment, divVec3, isLoop);
            }

            Debug.Log("x segCnt"+extendBezierControls.SegmentCount);
            Debug.Log("x length"+extendBezierControls.Points.Length);
