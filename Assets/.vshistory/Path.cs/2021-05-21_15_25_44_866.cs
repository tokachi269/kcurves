...
namespace Assets
{
    public class Path : MonoBehaviour
    {
        ...
        public Quaternion CalcRotation(int segIndex, float inputL)
        {
            ...
            {
                Knotndex = Beziers.SegmentCount - 1;
            }
            else
            {
                Knotndex = ((segIndex - 2) / 4 + 1) * 2 - 1;
            }

            int nextIndex = (Knotndex < Knots.Count ? segIndex + 1 : Knotndex);


            Quaternion rotation;
            if (!Quaternion.Equals(Knots[Knotndex].rotation, Knots[nextIndex].rotation))
            {
                rotation = Quaternion.Lerp(Knots[Knotndex].rotation, Knots[nextIndex].rotation, t);
            }
            else
            {
                rotation = Knots[Knotndex].rotation;
            }
            if (LookAts.Count != 0)
            {
                if (Knots[Knotndex].isLookAt || Knots[nextIndex].isLookAt)
                {
                    rotation = Quaternion.Lerp((Knots[Knotndex].isLookAt ? Quaternion.LookRotation(LookAts[0].position) : rotation),
