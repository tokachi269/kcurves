...
namespace Assets
{
    public class Path : MonoBehaviour
    {
        ...
        public Quaternion CalcRotation(int segIndex, float inputL)
        {
            ...
            knotIndex = (segIndex + 1) / 3 + 1;
            knotIndex - 1 = (segIndex + 1) / 3;


            knotNextIndex = (isLoop || knotIndex < Knots.Count -1 ? knotIndex + 1 : knotIndex);

            if(knotIndex == 0) sum += Beziers.Lengths[0, Beziers.ArcLengthWithTStep - 1];
            else if(knotIndex == Beziers.SegmentCount-1) sum += Beziers.Lengths[knotIndex, Beziers.ArcLengthWithTStep - 1];
            else sum += Beziers.Lengths[knotIndex, Beziers.ArcLengthWithTStep - 1];
            t = inputL/sum;
            ...
            if (t >= 0.99999f && segIndex == Beziers.SegmentCount - 1)
            {
