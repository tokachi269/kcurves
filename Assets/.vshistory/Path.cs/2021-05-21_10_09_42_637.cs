...
namespace Assets
{
    public class Path : MonoBehaviour
    {
        ...
        public Quaternion CalcRotation(int segIndex, float inputL)
        {
            ...
            int index = segIndex / 4 + 1;
            int nextIndex = (segIndex < Beziers.SegmentCount ? segIndex + 1 : segIndex);

            Quaternion rotation;
            if (!Quaternion.Equals(Knots[segIndex].rotation, Knots[nextIndex].rotation))
            {
                rotation = Quaternion.Lerp(Knots[segIndex].rotation, Knots[nextIndex].rotation, t);
            }
            else
            {
            ...
            if (LookAts.Count != 0)
            {
                if (Knots[segIndex].isLookAt || Knots[nextIndex].isLookAt)
                {
