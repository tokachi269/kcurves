    ...
    public class Path : MonoBehaviour
    {
        ...
        public Quaternion CalcRotation(int segIndex, float inputL)
        {
            ...
            if (LookAts.Count != 0)
            {
                ...
                {
                    rotation = Vector3.Lerp((_knots[knotIndex].isLookAt ? (Quaternion.LookRotation(LookAts[0].position) * new Quaternion(1, -1, 1, 1)).eulerAngles : rotation),
                                           (_knots[nextKnotIndex].isLookAt ? (Quaternion.LookRotation(LookAts[0].position) * new Quaternion(1, -1, 1, 1)).eulerAngles : rotation), t);
                }
            }
            rotation.z = 0f;
        ...
        {
            this.LookAts.RemoveAt(_knots.Count - 1);
            SetBezierFromKnots();
        }
