...
namespace Assets
{
    public class Path : MonoBehaviour
    {
        ...
        public void SetBezierFromKnots()
        {
            ...
            {
                extendBezierControls = new ExtendBezierControls(extendBezierControls.DividePoints(isLoop));
            }
            //tの移動距離を計算し、パラメータ化する
        ...
        public Vector3[] DividePoints(bool isLoop)
        {
            ...
            {
                Vector3[] result = BezierUtil.Divide(extendBezierControls[i, 0], extendBezierControls[i, 1], extendBezierControls[i + 1, 0], (float)Ts[i]);

                for (int j = 0; j <= 3; j++)
                {
                    dividedPoints[4 * i + j] = result[j];
                }
                dividedPoints[0] = extendBezierControls.Points[extendBezierControls.Points.Length - 1];
                extendBezierControls.Points = dividedPoints;
                extendBezierControls.SegmentCount = dividedSegmentCount;
            }
        }
