...
namespace Assets
{
    public class Path : MonoBehaviour
    {
        ...
        public bool _isLoop { get; private set; }
        ...
        {
            _beziers = KCurves.CalcBezier(_knots, _isLoop) as ExtendBezierControls;
        }
        ...
        public void MoveCamera()
        {
            ...
            _maxSpeed = MaxSpeed(_time);
            Debug.Log("maxSpeed" + _maxSpeed);

            _knotIndex = _bezierIndex % 2 == 0 ? _bezierIndex + 1 / 2 : _bezierIndex / 2;

            _mode = (EasingMode)((byte)_knots[_knotIndex].easingMode | ((byte)_knots[_knotIndex + 1].easingMode << 1));
            Debug.Log(_mode);
            ...
            if (_knotIndex != 0)
            {
                for (ushort j = (ushort)(2 * _knotIndex - 1); j <= 2 * _knotIndex && _knotIndex + 1 < _knots.Count; j++)
                {
            ...
            float easing = Easing.GetEasing(_mode, (_bezierIndex != 0 && _bezierIndex % 2 == 0 ? _ProgressLength - _beziers.Length(2 * _knotIndex - 1) : _ProgressLength) / KnotBetweenRange);
            Debug.Log(easing);
            ...
            //Debug.Log("seg:" + bezierIndex + "  inputL:" + inputL + "maxS:" + maxSpeed + "currentTime:" + currentTime);
            {
                ...
                //Debug.Log("t:" + diffT);
                befT = _t;
                Vector3 now = CalcPosition(_isLoop, _t);
            ...
            {
                Vector3 pos = CalcPosition(_isLoop, _t);
            ...
            int i = (int)Math.Floor(_currentTime / _time * _beziers.SegmentCount);
        ...
        {
            if (!_beziers.IsCalcArcLengthWithT) _beziers.CalcArcLengthWithT(_isLoop);
        ...
        public void GetBeforeAndBehindKnot(int segIndex, out ushort knotIndex, out ushort nextKnotIndex)
        {
            ...
            nextKnotIndex = ((ushort)(_isLoop || knotIndex < _knots.Count - 1 ? knotIndex + 1 : knotIndex));
        }
    }
}