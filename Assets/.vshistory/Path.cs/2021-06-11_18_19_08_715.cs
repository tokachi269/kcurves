...
namespace Assets
{
    public class Path : MonoBehaviour
    {
        ...
        public Quaternion CalcRotation(int segIndex, float inputL)
        {
            ...
            if (!Quaternion.Equals(Knots[segIndex].rotation, Knots[segIndex+1].rotation))
            {
                Debug.Log("Knots[knotIndex].rotation.eulerAngles" + Knots[segIndex].rotation.eulerAngles);
                Debug.Log("Knots[knotNextIndex].rotation.eulerAngles" + Knots[segIndex+1].rotation.eulerAngles);
                if (t <= 0f && segIndex == 0)
                {
                ...
                {
                    rotation = Vector3.Lerp(Knots[segIndex].rotation.eulerAngles, ClosestAngle(Knots[segIndex].rotation.eulerAngles, Knots[segIndex+1].rotation.eulerAngles), t);
                }
            }
            else
            {
                rotation = Knots[segIndex].rotation.eulerAngles;
            }
            if (LookAts.Count != 0)
            {
                if (Knots[segIndex].isLookAt || Knots[segIndex+1].isLookAt)
                {
                    rotation = Vector3.Lerp((Knots[segIndex].isLookAt ? (Quaternion.LookRotation(LookAts[0].position) * new Quaternion(1, -1, 1, 1)).eulerAngles : rotation),
                                           (Knots[segIndex+1].isLookAt ? (Quaternion.LookRotation(LookAts[0].position) * new Quaternion(1, -1, 1, 1)).eulerAngles : rotation), t);
                }
            }
            rotation.z = 0f;
