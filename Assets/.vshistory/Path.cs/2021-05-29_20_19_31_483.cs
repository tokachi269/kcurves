...
namespace Assets
{
    public class Path : MonoBehaviour
    {
        ...
        ushort knotIndex = 0, nextKnotIndex = 1;
        EasingMode mode;
        ...
        public void MoveCamera()
        {
            ...
            float length = 0;
                ...
                float easing = Easing.GetEasing(mode, (inputL) / Beziers.Lengths[nextKnotIndex - 1, Beziers.ArcLengthWithTStep - 1]);
                
                t = Beziers.GetT(bezierIndex, out nextBezierIndex, ref easing);
                if (bezierIndex == nextBezierIndex)
                {
                    for (int j = 2 * knotIndex - 1; j <= 2 * knotIndex && knotIndex + 1 < Knots.Count; j++)
                    {
                        length += Beziers.Lengths[j, Beziers.ArcLengthWithTStep - 1];
                    }
