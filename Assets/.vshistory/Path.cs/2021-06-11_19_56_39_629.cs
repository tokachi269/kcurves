...
namespace Assets
{
    public class Path : MonoBehaviour
    {
        ...
        public IEnumerator MoveCamera()
        {
            ...
            Debug.Log(easingMode.Length);
            foreach(var e in easingMode)
            {
                Debug.Log(e);
            }
            for (float currentTime= 0; ;)
            {
                ...
                mode = (EasingMode)((byte)easingMode[knotIndex] | ((byte)easingMode[knotIndex + 1] << 1));
                ...
                float easing = Easing.GetEasing(easingMode[knotIndex], ProgressLength, KnotBetweenRange);
        ...
        public Quaternion CalcRotation(int knotIndex, float ratio)
        {
            ...
            if (!Quaternion.Equals(Knots[knotIndex].rotation, Knots[knotIndex+1].rotation))
            {
                ...
                {
                    rotation = Vector3.Lerp(Knots[knotIndex].rotation.eulerAngles, ClosestAngle(Knots[knotIndex].rotation.eulerAngles, Knots[knotIndex+1].rotation.eulerAngles), ratio);
                }
            }
            else
            {
