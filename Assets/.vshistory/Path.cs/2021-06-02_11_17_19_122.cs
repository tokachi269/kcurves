...
namespace Assets
{
    public class Path : MonoBehaviour
    {
        ...
        public void MoveCamera()
        {
            ...
            float progressre = ProgressLength;

            Debug.Log("KnotBetweenRange:" + KnotBetweenRange);
            if (knotIndex != 0)
            {
                for (int k = 0; k < knotIndex; k++)
                {
                    progressre -= Beziers.Length(k);
                }
            }

            if (bezierIndex % 2 == 1 || (bezierIndex == 0 && progressre > Beziers.Length(bezierIndex)))
            {
                if (progressre > Beziers.Length(bezierIndex)) 
                {
                    bezierIndex++;
                    Debug.Log("bezierIndex++");
                }
            }
            else if (bezierIndex == 0 && progressre > Beziers.Length(bezierIndex)) 
            {
                bezierIndex++;
                progressre -= Beziers.Length(bezierIndex - 1) + Beziers.Length(bezierIndex);
                Debug.Log("knotIndex++");

            }else if (bezierIndex != 0 && progressre > Beziers.Length(bezierIndex - 1) + Beziers.Length(bezierIndex))
            {
                bezierIndex++;
                progressre -= Beziers.Length(bezierIndex - 1) + Beziers.Length(bezierIndex);
                Debug.Log("knotIndex++");
            ...
            {
                d -= Beziers.Length(2 * knotIndex - 1);
            }

            d /= KnotBetweenRange;

            float easing = Easing.GetEasing(mode, d);
            //Debug.Log("easing:"+easing);

            if (bezierIndex != 0 && bezierIndex % 2 == 0) {
                easing = (easing - Beziers.Length(2 * knotIndex - 1)) * KnotBetweenRange;
            }
            else
            {
                easing *= KnotBetweenRange;
            }
