...
namespace Assets
{
    public class Path : MonoBehaviour
    {
        ...
        //ユーザー制御点
        private List<ControlPoint> Knots = new List<ControlPoint>();

        //ユーザー制御注視点
        private List<ControlPoint> LookAts = new List<ControlPoint>();

        //Bezier計算結果
        private ExtendBezierControls Beziers;

        private bool isLoop { get; private set; }
        ...
        public void MoveCamera()
        {
            ...
            knotIndex = bezierIndex % 2 == 0 ? bezierIndex + 1 / 2 : bezierIndex / 2;
            ...
            if (bezierIndex % 2 == 1)
            {
                if (ProgressLength < Beziers.Length(bezierIndex)) bezierIndex++;
            }
            else if (bezierIndex!=0 && ProgressLength < Beziers.Length(bezierIndex - 1) + Beziers.Length(bezierIndex)) 
            {
                bezierIndex++;
            ...
            float easing = Easing.GetEasing(mode, (bezierIndex != 0 && bezierIndex % 2 == 0 ? ProgressLength - Beziers.Length(2 * knotIndex - 1) : ProgressLength) / KnotBetweenRange);
            Debug.Log(easing);
            t = Beziers.GetT(bezierIndex, easing * KnotBetweenRange);
