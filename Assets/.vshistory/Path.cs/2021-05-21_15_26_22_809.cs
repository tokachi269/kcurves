...
namespace Assets
{
    public class Path : MonoBehaviour
    {
        ...
        public Quaternion CalcRotation(int segIndex, float inputL)
        {
            ...
            int knotNextIndex = (knotIndex < Knots.Count ? segIndex + 1 : knotIndex);


            Quaternion rotation;
            if (!Quaternion.Equals(Knots[knotIndex].rotation, Knots[knotNextIndex].rotation))
            {
                rotation = Quaternion.Lerp(Knots[knotIndex].rotation, Knots[knotNextIndex].rotation, t);
            }
            else
            {
            ...
            if (LookAts.Count != 0)
            {
                if (Knots[knotIndex].isLookAt || Knots[knotNextIndex].isLookAt)
                {
                    rotation = Quaternion.Lerp((Knots[knotIndex].isLookAt ? Quaternion.LookRotation(LookAts[0].position) : rotation),
                                           (Knots[knotNextIndex].isLookAt ? Quaternion.LookRotation(LookAts[0].position) : rotation), t);
                }
            }
            return rotation;
        }
