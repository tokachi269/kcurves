...
namespace Assets
{
    public class ExtendBezierControls : BezierControls
    {
        ...
        internal float GetT(int bezierIndex, out int nextSegIndex, ref float inputL)
        {
            if (inputL <= 0)
            {
                nextSegIndex = bezierIndex;
                return 0f;
            }
        ...
        public void CalcArcLengthWithT(int count ,bool isLoop)
        {
            ...
            TotalLength = Lengths.Cast<int>().Where((n, i) => i != 0 && (i % ArcLengthWithTStep - 1) == 0).Sum();
