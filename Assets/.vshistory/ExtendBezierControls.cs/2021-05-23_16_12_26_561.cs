...
namespace Assets
{
    public class ExtendBezierControls : BezierControls
    {
        ...
        internal float GetT(int segaIndex, out int nextSegIndex, ref float inputL)
        {
            if (inputL <= 0) return 0f;
            if ( inputL >= Lengths[nextSegIndex, ArcLengthWithTStep - 1])
            {
                if (nextSegIndex <= SegmentCount - 1)
                {
                    inputL -= Lengths[nextSegIndex, ArcLengthWithTStep - 1];
                    nextSegIndex++;
                    if (nextSegIndex == SegmentCount)
                    {
                        return SegmentCount;
                    }
                }
            }
            int index = Mathf.Clamp((int)Math.Floor(inputL / Lengths[nextSegIndex, ArcLengthWithTStep - 1] * (ArcLengthWithTStep - 1)),0, ArcLengthWithTStep - 1);

            for (int i = 0; i < ArcLengthWithTStep - 1; i++)
            {
                if ((index <= 0) || (index >= ArcLengthWithTStep - 1)) break;
                if (inputL <= Lengths[nextSegIndex, index])
                    if (Lengths[nextSegIndex, index - 1] < inputL) break;
                    else index--;
                else index++;
            }
            //Debug.Log("segIndex"+ segIndex + "  index:" + index);
            //Debug.Log("  inputL:" + inputL+ "  indexL:" + Lengths[segIndex, index]+ "  index:" + index);
            float resultL = 1+index - ((Lengths[nextSegIndex, index] - inputL) / (Lengths[nextSegIndex, index] - (index <= 0 ? 0 : Lengths[nextSegIndex, index - 1])));
            float resultT = nextSegIndex + (float)(resultL / ArcLengthWithTStep);
            return resultT;
        }
