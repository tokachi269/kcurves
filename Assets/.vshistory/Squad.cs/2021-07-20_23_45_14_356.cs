...
namespace Assets
{
    class Squad
    {
        ...
        {
			int section = (knots.Length - 1) * t;
			float alongLine = (knots.Length - 1) * t - section;
			
			if (section == 0)
			{
				return SplineSegment(knots[section], knots[section], knots[section + 1], knots[section + 2], alongLine);

			}
			else if (section == knots.Length - 2 and section > 0){
				return SplineSegment(knots[section - 1], knots[section], knots[section + 1], knots[section + 1], alongLine);

			}
			else if (section >= 1 and section < quaternions.Length - 2){
				return SplineSegment(knots[section - 1], knots[section], knots[section + 1], knots[section + 2], alongLine);
