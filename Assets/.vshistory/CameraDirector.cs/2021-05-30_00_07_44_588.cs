...
namespace Assets
{
    public class CameraDirector : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            moveCameraCube.transform.position = path._knots[0].position;
        ...
        void Update()
        {
            ...
            if (isStart)
            {
                if (path._knots.Count > 1 && moveCameraCube != null)
                {
                    path.MoveCamera();
                }
        ...
        public void renderObject()
        {
            if (!(path is null))
            {
                ...
                {
                    for (int i = 0; i < path._knots.Count - 1; i++)
                    {
                        inputCube[i].transform.position = path._knots[i].position;
                        inputCube[i].transform.rotation = path._knots[i].rotation;
                    }
                }
                if (path._knots.Count > inputCube.Count)
                {
