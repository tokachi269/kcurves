            ...
            if (isStart)
            {
                ...
                if (path.Knots.Count > 1 && moveCameraCube != null)
                {
                    ...
                    for (byte n = 0; n <= 3; n++)
                    {
                        for (byte m =0; m <= 3 ; m++)
                        {
                            Debug.Log((EasingMode)(Math.Pow(4, n)) + "," + (EasingMode)(Math.Pow(4, m)) + "= " + Convert.ToString((byte)Math.Pow(4, n) | ((byte)Math.Pow(4, n) << 1), 2).Format("{0:D4}", num));
                        }
                    }
                    Debug.Log("" + Convert.ToString(((byte)EasingMode.Auto << 1), 2));
