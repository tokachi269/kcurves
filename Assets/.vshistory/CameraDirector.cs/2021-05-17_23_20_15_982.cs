    ...
    public class CameraDirector : MonoBehaviour
    {
        ...
        public void renderObject()
        {
            if (!(path is null))
            {
                ...
                for (int i = 0; i < path.extendBezierControls.SegmentCount - 1; i++)
                {
                    bezierObject.Add(new GameObject("bezierControl" + i));
                    bezierObject[i] = GameObject.CreatePrimitive(PrimitiveType.Cube);
                    bezierObject[i].transform.position = path.extendBezierControls.Points[i*2];

                    bezierObject[i].transform.localScale = new Vector3(0.2f, 0.2f, 0.2f);
                    bezierObject[i].transform.parent = this.transform;
                    bezierObject[i].GetComponent<Renderer>().material.color = Color.red;
                }
                bezierObject.Add(new GameObject("bezierControl last"));
                bezierObject[bezierObject.Count-1] = GameObject.CreatePrimitive(PrimitiveType.Cube);
                bezierObject[bezierObject.Count - 1].transform.position = path.extendBezierControls.Points[path.extendBezierControls.Points.Length-1];

                bezierObject[bezierObject.Count - 1].transform.localScale = new Vector3(0.2f, 0.2f, 0.2f);
                bezierObject[bezierObject.Count - 1].transform.parent = this.transform;
                bezierObject[bezierObject.Count - 1].GetComponent<Renderer>().material.color = Color.red;
