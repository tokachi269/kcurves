...
namespace Assets
{
    public class CameraDirector : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            if (isStart)
            {
                ...
                if (path.Knots.Count > 1 && moveCameraCube != null)
                {
                    ...
                    t = path.Beziers.GetT(out nextSegIndex, ref inputL);
                    if (segIndex == nextSegIndex)
                    {
                        int knotIndex, nextKnotIndex;
                        path.GetBeforeAndBehindKnot(segIndex, out knotIndex, out nextKnotIndex);

                        EasingMode mode = (EasingMode)((byte)path.Knots[knotIndex].easingMode | ((byte)path.Knots[nextKnotIndex].easingMode << 1));

                        currentMode = GetEaseInOut(mode);
                    }
                    //Debug.Log(t);
        ...
        private EasingMode GetEaseInOut(EasingMode mode)
        {
            float num1 = 0;
            switch (mode)
            {
