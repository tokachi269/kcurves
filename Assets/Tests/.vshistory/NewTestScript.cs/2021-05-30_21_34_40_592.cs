...
namespace Assets
{
    public class NewTestScript
    {
        ...
        [Test]
        public void CameraMoveTest()
        {
            var cd = new GameObject("Hoge");
            CameraDirector cameraDirector = go.gameObject.AddComponent<CameraDirector>();

            cameraDirector.AddKnot(new Vector3(0, 0, 0), new Quaternion(0, 0, 0, 1), 60, false);
            cameraDirector.AddKnot(new Vector3(1, 2, 1), new Quaternion(0, 0.5f, 0, 1), 60, false);
            cameraDirector.AddKnot(new Vector3(0, 2, 5), new Quaternion(0, 0, 0, 1), 60, true);
            cameraDirector.AddKnot(new Vector3(0, -2, 1), new Quaternion(0, 0, 1, 1), 60, false);
            cameraDirector.AddKnot(new Vector3(0, 1, 4), new Quaternion(0, 0, 0, 1), 60, false);
            cameraDirector.AddLookAt(new Vector3(0, 4, 0), new Quaternion(0, 0, 0, 1), 60);
            ...
            path.Beziers.CalcArcLengthWithT(isLoop: false);
            CameraDirector()
            Assert.IsTrue(0 <= path.diffT);
            Assert.IsTrue(0 < path.dist);
        }
    }
}