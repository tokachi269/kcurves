...
namespace Assets
{
    public class ExtendBezierControls : BezierControls
    {
        ...
        {
            int segCnt = isLoop || SegmentCount < 3 ? SegmentCount+1 : SegmentCount - 2;
            ...
            for (i = 0; i < segCnt; i++)
            {
                ...
                var nextk = i % SegmentCount;
                for (var j = 0; j < stepPerSegment; j++)
                {
                    t = j / (float)stepPerSegment;
                    plots[offset + j] = BezierUtil.Position(this[nextk, 0], this[nextk, 1], this[nextk, 2], t);
                }
            }
