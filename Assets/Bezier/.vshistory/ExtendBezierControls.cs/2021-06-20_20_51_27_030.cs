...
namespace Assets
{
    public class ExtendBezierControls : BezierControls
    {
        ...
        public Vector3[] CalcPlots(ushort stepPerSegment, bool isLoop)
        {
            ...
            Vector3[] plots = new Vector3[Math.Max(0, segCnt) * stepPerSegment + 1];
            ...
            for (i = 0; i < segCnt; i++)
            {
                ...
                int nexti = (isLoop && i == segCnt - 1) ? 0 : i;

                for (var j = 0; j < stepPerSegment; j++)
                {
                    t = j / (float)stepPerSegment;
                    plots[offset + j] = BezierUtil.Position(this[i, 0], this[i, 1], this[nexti, 2], t);
                }
            }
            var last = isLoop || 1<segCnt ? 0 : i - 1;
            if (isLoop)
            {
                offset = i * stepPerSegment;
                ...
                {
                    t = j / (float)stepPerSegment;
                    plots[offset + j] = BezierUtil.Position(this[i, 0], this[i, 1], this[last, 0], t);
                }
            }
            else
            {
                plots[plots.Length - 1] = BezierUtil.Position(this[last, 0], this[last, 1], this[last, 0], 1);
