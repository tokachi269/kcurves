...
namespace Assets.Path
{
    ...
    {
        private override string Name { get; set; }

        //ユーザー制御点
        private override List<ControlPoint> Knots { get; set; } = new List<ControlPoint>();
        public int KnotsCount => Knots.Count;

        //ユーザー制御注視点
        private List<ControlPoint> LookAts { get; set; } = new List<ControlPoint>();
        public int LookAtsCount => Knots.Count;

        //Bezier計算結果
        private ExtendBezierControls Beziers { get;  set; }
        public int BeziersCount => Knots.Count;
