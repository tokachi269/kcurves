            ...
            for (float currentTime= 0; ;)
            {
                ...
                else
                {
                    if (BezierProgressLength >= Beziers.Length(bezierIndex - 1) + Beziers.Length(bezierIndex))
                    {
                        ...
                        KnotProgressLength -= Beziers.Length(bezierIndex - 1) + Beziers.Length(bezierIndex);
                        isSegChanged = true;
                    }
                }
                ...
                float easingLength = Easing.GetEasing(mode, KnotProgressLength, KnotBetweenRange);
